...
public final class TilePatternObjective implements Objective {
    ...
    private List<List<Element>> generateRotations(List<Element> pattern) {
        var allRotations = new ArrayList<List<Element>>();
        allRotations.add(pattern);

        // rotate the pattern to find all possible patterns
        final int ROTATION_COUNT = 6;
        for (int i = 1; i < ROTATION_COUNT; ++i) {
            var currentRotation = new ArrayList<Element>();
            currentRotation.add(pattern.get(0));
            for (int j = 1; j < pattern.size(); ++j) {
                currentRotation.add(
                        allRotations.get(i - 1).get(j).rotate(currentRotation.get(0).coord()));
            }
            allRotations.add(currentRotation);
        }
        return allRotations;
    }
    ...
    private List<List<Element>> generateShifts(List<Element> pattern) {
        var allShifts = new ArrayList<List<Element>>();
        for (int i = 0; i < pattern.size(); ++i) {
            var offset = pattern.get(i).coord().negated();
            var currentShift = pattern.stream().map(e -> e.offset(offset)).toList();
            allShifts.add(currentShift);
        }
        return allShifts;
    }
